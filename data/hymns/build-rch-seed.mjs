// Regenerate data/hymns/pcn-hymns-seed.json from the RCH PLUS source document.
//
// The .docx is a zip; first extract its main part, then run this script:
//   unzip -o "RCH PLUS completed  WORKING.docx" word/document.xml -d /tmp/rch
//   node data/hymns/build-rch-seed.mjs /tmp/rch/word/document.xml
//
// Then import into the DB with:  bun run data/build-hymns.ts
import { readFileSync, writeFileSync } from 'node:fs'
import { dirname, join } from 'node:path'

const XML_PATH = process.argv[2] || 'word/document.xml'
const OUT_PATH = join(dirname(new URL(import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, '$1')), 'pcn-hymns-seed.json')
...
const out = {
  // Canonical PCN hymnal seed: the full Revised Church Hymnary (RCH) plus the
  // RCH+ gospel-song supplement, parsed from "RCH PLUS completed WORKING.docx".
  hymnal: {
    slug: 'pcn',
  ...
  _note:
    'Full RCH 1-713 + RCH+ 714-881 supplement, generated from the source docx ' +
    'by data/hymns/_rch_work/parse.mjs. Source typos fixed: RCH+ 7567 -> 757, ' +
    'duplicate 860 -> 881.',
  hymns,
}
